...
class UgBuddies extends ConsumerStatefulWidget {
  const UgBuddies({super.key});

  @override
  ConsumerState<UgBuddies> createState() => _UgBuddiesState();
}

class _UgBuddiesState extends ConsumerState<UgBuddies> {
  ...
  Map<String, List> dataMap = {};
    ...
    for (var posItem in pos) {
      futures.add(FirestoreData.getSpecificData('ug-buddy',posItem));
    }
  ...
  addDataToProvider() async {
    ...
    for (int i = 0; i < dataMap.length; i++) {
      ...
      print(data);
      ref.read(ugBuddyDataProvider.notifier).addAllData(pos[i], data);
    }
  }

  isFirstOpen() async {
    if (ref.read(ugBuddyDataProvider)[pos[0]] == null) {
  ...
  Widget build(BuildContext context) {
    final dataMap = ref.watch(ugBuddyDataProvider);
    List setData =[];
