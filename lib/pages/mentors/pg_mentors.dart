...
class PgMentors extends ConsumerStatefulWidget {
  const PgMentors({super.key});

  @override
  ConsumerState<PgMentors> createState() => _PgMentorsState();
}

class _PgMentorsState extends ConsumerState<PgMentors> {
  ...
  getData() async {
    ...
    for (var posItem in pos) {
       futures.add(FirestoreData.getSpecificData('pg-mentor',posItem));
    }
  ...
  addDataToProvider() async {
    ...
    for (int i = 0; i < dataMap.length; i++) {
      ...
      print(data);
      ref.read(pgMentorDataProvider.notifier).addAllData(pos[i], data);
    }
  }

  isFirstOpen() async {
    if (ref.read(pgMentorDataProvider)[pos[0]] == null) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Column(
        children: [
          const Headers(title: 'PG Mentors'),
          const SizedBox(
            height: 25.0,
          ),
          isLoading? 
